...
public class SSIM_index_modified
{
    ...
    public void run(ImageProcessor imageA, ImageProcessor imageB)
    {
        int maxWidth = Math.max(imageA.getWidth(), imageB.getWidth());
        int maxHeight = Math.max(imageA.getHeight(), imageB.getHeight());

        imageA = imageA.resize(maxWidth, maxHeight);
        imageB = imageB.resize(maxWidth, maxHeight);

        this.image_1_imp = new ImagePlus("1", imageA);
        this.image_2_imp = new ImagePlus("1", imageB);

        int image_height = this.image_1_imp.getHeight();
        int a = this.image_2_imp.getHeight();
        if (a != image_height) {
        ...
        for (int pointer = 0; pointer < image_dimension; pointer++)
        {
            ...
            int tmp1246_1243 = a; array_mu1_ip_copy[pointer] = tmp1246_1243; array_mu1_ip[pointer] = tmp1246_1243;
            int tmp1260_1257 = b; array_mu2_ip_copy[pointer] = tmp1260_1257; array_mu2_ip[pointer] = tmp1260_1257;
        }
